...
void stutter_lockme(t_stutter *x, long n);
...
//t_int *stutter_perform(t_int *w)
void stutter_attach_buffer(t_stutter *x);
float erand(void);
	...
	if(x->mute_me || ! x->b_valid) {
		...
		while( n-- ) {
			...
			if( samps_to_go > loop_samps - taper_samps ){
				*out1++ = tab[ b_index ] * ( (float)(loop_samps - samps_to_go)/(float)taper_samps );
				++b_index;
			} else if( samps_to_go < taper_samps ) {
				*out1++ = tab[ b_index ] * ( (float)(samps_to_go)/(float)taper_samps );
				++b_index;
